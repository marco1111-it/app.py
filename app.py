...
if codice:
    ...
    df_b_pairs = (
        ...
        .agg({"QUANTITà": "sum", "FAMIGLIA": "first"})
    )

    df_lotti_2 = (
        df_b_pairs.groupby("LOTTO")
        .filter(lambda x: x["CODICE"].nunique() == 2)
    )

    per_lotto = (
        df_lotti_2.groupby("LOTTO")
        .apply(lambda x: pd.Series({
            "COPPIA": "-".join(sorted(x["CODICE"].unique())),
    ...
    # ⚠️ QUI FIX IMPORTANTE: uso df_b_raw NON df_b normalizzato
    qta_lotti = (
        df_b_raw[df_b_raw["CODICE"].astype(str).str.contains(codice, na=False)]
        .groupby("LOTTO", as_index=False)["QUANTITà"]
        .sum()
    )
    ...
    if df_merge.empty:
        ...
        pezzi_stimati = ore_teoriche / ore_per_pezzo if ore_per_pezzo > 0 else 0

        rate_teorico = pezzi_stimati / ore_teoriche if ore_teoriche > 0 else 0
        ...
        st.write({
            ...
            "ORE_PER_PEZZO": round(ore_per_pezzo, 4),
